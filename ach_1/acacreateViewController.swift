...
protocol acareate_delegate{
    func accreate_event(flag:Bool)
}
import UIKit

class acacreateViewController:UIViewController ,UITextFieldDelegate,UIToolbarDelegate,NSURLSessionDelegate,NSURLSessionDataDelegate{
    //error(まだつかってない)
    enum createpostE:ErrorType{
        case Nil
                            ...
                            if(cookie.name == "sessionID"){
                              ...
                              // 更新はmain threadで
                                    dispatch_async(q_main, {
                                        self.performSegueWithIdentifier("AccountCreate_to_MyPage", sender: self)
                                    })
                            }
                        }
                        } else{
    ...
    func pickerValueChange(sender:UIDatePicker){
        let selectdata = String(sender.date)
        var Dcount = 0
        var count = 0
        var countfag = 0
       ...
       print(sender.date)
        while  true{
            ...
            if selectdata[selectdata.startIndex.advancedBy(count)] == "-"{
                if Dcount==0{
